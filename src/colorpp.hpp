...
inline auto enable_colors() -> void { color_state_storage() = true; }
inline auto disable_colors() -> void { color_state_storage() = false; }
...
#define ANSI_GEN(name, code)                                                   \
  ...
  [[nodiscard]] inline auto name(std::string_view text) -> std::string {       \
    ...
    std::string out;                                                           \
    out.reserve(colors::ansi::name.size() + text.size() +                      \
                colors::ansi::reset.size());                                   \
